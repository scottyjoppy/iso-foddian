...
TileBounds::TileBounds() :
    ...
    m_origin(0.f, 0.f) // default origin at (0,0)
{
}
...
void TileBounds::SetOrigin(const sf::Vector2f& origin)
{
    m_origin = origin;
}
...
{
    sf::Vector2f p = pos + m_origin;

    m_top.setPointCount(4);
    m_top.setPoint(0, {p.x, p.y - height});
    m_top.setPoint(1, {p.x + size.x / 2, p.y + size.y / 2 - height});
...
void TileBounds::BuildBottom(const sf::Vector2f& pos, const sf::Vector2f& size)
{
    ...
    m_bottom.setPoint(0, {p.x, p.y});
    m_bottom.setPoint(1, {p.x + size.x / 2, p.y + size.y / 2});
...
void TileBounds::BuildWallLeft(const sf::Vector2f& pos, const sf::Vector2f& size, float height)
{
    ...
    m_left.setPoint(0, {p.x - size.x / 2, p.y + size.y / 2});
    m_left.setPoint(1, {p.x, p.y + size.y});
...
void TileBounds::BuildWallRight(const sf::Vector2f& pos, const sf::Vector2f& size, float height)
{
    ...
    m_right.setPoint(0, {p.x, p.y + size.y});
    m_right.setPoint(1, {p.x + size.x / 2, p.y + size.y / 2});
...
void TileBounds::BuildWallBackL(const sf::Vector2f& pos, const sf::Vector2f& size, float height)
{
    ...
    m_backL.setPoint(0, {p.x, p.y});
    m_backL.setPoint(1, {p.x - size.x / 2, p.y + size.y / 2});
...
void TileBounds::BuildWallBackR(const sf::Vector2f& pos, const sf::Vector2f& size, float height)
{
    ...
    m_backR.setPoint(0, {p.x, p.y});
    m_backR.setPoint(1, {p.x + size.x / 2, p.y + size.y / 2});
...
std::vector<sf::Vector2f> TileBounds::GetVertices() const
{
    ...
    for (size_t i = 0; i < m_top.getPointCount(); i++)
    {
        verts.push_back(m_top.getTransform().transformPoint(m_top.getPoint(i)));
    }
    return verts;
}
